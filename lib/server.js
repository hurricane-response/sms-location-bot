...
app.post('/sms', (req, res) => {
  ...
  if(sentZipcodes.length == 0) {
    ...
    for (let sentZipcode of sentZipcodes) {
      console.log(`Checking zip: ${sentZipcode}`)
      let sentZipInfo = zipcodes.lookup(sentZipcode);
      let zipLatLon = new LatLon(sentZipInfo.latitude, sentZipInfo.longitude);
      for (let msg of sheltersFinder.findShelters(zipLatLon)) {
